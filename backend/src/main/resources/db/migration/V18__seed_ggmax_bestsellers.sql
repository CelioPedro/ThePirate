...
(gen_random_uuid(), 'TPM-ROBLOX-1000-001', 'Robux - 1000 Moedas (Via Grupo)', 'Saldo inserido diretamente na sua conta via repasse de grupo no Roblox.', 'ACTIVE', 4500, 'BRL', 'MANUAL', false, now(), now(), 'roblox-1000-robux', 'ASSINATURA', (select id from catalog_categories where slug = 'gift-cards'), 'ROBLOX', 'BR', 0, 'Após a compra, informe o nome da sua conta e entre no nosso grupo.'),
(gen_random_uuid(), 'TPM-VALORANT-POINTS-001', 'Valorant Points', 'Código de recarga para adicionar saldo à sua conta Valorant (Servidor BR).', 'ACTIVE', 3990, 'BRL', 'MANUAL', false, now(), now(), 'valorant-points-card', 'ASSINATURA', (select id from catalog_categories where slug = 'gift-cards'), 'RIOT', 'BR', 0, 'O PIN será revelado nos detalhes do seu pedido após o processamento.'),
...
(gen_random_uuid(), 'TPM-WIN11-PRO-001', 'Windows 11 Pro - Chave Digital', 'Chave de Ativação (Key) Original OEM para uso em apenas 1 PC. Vitalício.', 'ACTIVE', 2990, 'BRL', 'MANUAL', false, now(), now(), 'windows-11-pro-key', 'ASSINATURA', (select id from catalog_categories where slug = 'softwares-licencas'), 'MICROSOFT', 'BR', 0, 'Chave 25 dígitos enviada para uso imediato.'),
(gen_random_uuid(), 'TPM-OFFICE365-001', 'Office 365 Anual (Conta)', 'Conta com acesso garantido aos aplicativos Office (Word, Excel, PowerPoint) e 1TB no OneDrive.', 'ACTIVE', 4990, 'BRL', 'MANUAL', false, now(), now(), 'office-365-anual', 'ASSINATURA', (select id from catalog_categories where slug = 'softwares-licencas'), 'MICROSOFT', 'BR', 365, 'Receba uma conta exclusiva configurada para uso nos apps Office.'),
